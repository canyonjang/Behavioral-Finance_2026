...
# ---------------------------------------------------------
# 3. 이번 주 설정 (매주 수업 전에 이 두 가지만 수정하세요)
# ---------------------------------------------------------
CURRENT_WEEK = "1주차"  # 저장될 탭 이름
        ...
        if submitted:
            if not name or not student_id:
                ...
                try:
                    # --- A. 주차별 탭 저장 ---
                    try:
                        week_df = conn.read(worksheet=CURRENT_WEEK)
                    ...
                    # --- B. 전체데이터 탭 저장 ---
                    master_df = conn.read(worksheet="전체데이터")
                    ...
                    st.error(f"저장 실패. 구글 시트의 탭 이름과 편집 권한을 확인하세요. ({e})")
...
# --- [TAB 3] 누적 성적 분석 ---
with tab3:
    st.header("📊 학생별 누적 정답 비중")
    try:
        data = conn.read(worksheet="전체데이터")
        if not data.empty:
            # 기간 분류 (중간고사 기준일: 2026-04-20)
